...
exports.TaxonomyNavigator = void 0;
...
class TaxonomyNavigator {
    ...
    constructor(config = {}) {
        ...
        // Get API key
        this.config.apiKey = (0, config_1.getApiKey)(this.config.apiKey);
        // Initialize OpenAI
     ...
     * @private
     */
    async generateProductSummary(productInfo) {
...
Summary:`;
        try {
            const response = await this.callOpenAI('You are a product categorization assistant. Always use the most common, standard product name (e.g., \'television\' not \'display device\'). Include helpful synonyms in parentheses. Be direct and avoid flowery descriptions.', prompt, 100);
     ...
     * @private
     */
    async stage1SelectL1Categories(productSummary) {
        // Extract unique L1 categories
        const l1Categories = [...new Set(this.allPaths
                .filter(p => p.isLeaf)
                .map(p => p.parts[0]))];
...
Return one category per line:`;
        try {
            const response = await this.callOpenAI('You are a product categorization assistant. Select L1 categories from the provided list using exact spelling.', prompt);
     ...
     * @private
     */
    async stage2SelectLeaves(productSummary, selectedL1s, excludedLeaves, stageName) {
...
15`;
            try {
                const response = await this.callOpenAI('You are a product categorization assistant. Select categories by their numbers only. Return only numbers, one per line.', prompt);
     ...
     * @private
     */
    async stage3FinalSelection(productSummary, leaves) {
...
The number must be between 1 and ${leaves.length}.`;
        try {
            const response = await this.callOpenAI('You are a product categorization assistant. Select the single best matching category by its number.', prompt, 150, this.config.stage3Model);
     ...
     * @private
     */
    async callOpenAI(systemPrompt, userPrompt, maxTokens = 150, model) {
        this.apiCallCount++;
     ...
     */
    buildTaxonomyTree() {
        const content = (0, fs_1.readFileSync)(this.config.taxonomyFile, 'utf-8');
        ...
        lines.forEach((line, index) => {
            ...
            const isLeaf = !lines.slice(index + 1).some(nextLine => nextLine.trim().startsWith(line + ' > '));
     ...
     */
    convertLeavesToPaths(leaves) {
        return leaves.map(leaf => {
            const path = this.allPaths.find(p => p.isLeaf && p.parts[p.parts.length - 1] === leaf);
...
exports.TaxonomyNavigator = TaxonomyNavigator;
/**
