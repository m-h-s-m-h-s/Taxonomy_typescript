...
exports.TaxonomyInterface = void 0;
exports.main = main;
...
const logger = {
    debug: (msg) => currentLogLevel <= LogLevel.DEBUG && console.log(`[${new Date().toISOString()}] - taxonomy_interface - DEBUG - ${msg}`),
    info: (msg) => currentLogLevel <= LogLevel.INFO && console.log(`[${new Date().toISOString()}] - taxonomy_interface - INFO - ${msg}`),
    warning: (msg) => currentLogLevel <= LogLevel.WARNING && console.warn(`[${new Date().toISOString()}] - taxonomy_interface - WARNING - ${msg}`),
    error: (msg) => currentLogLevel <= LogLevel.ERROR && console.error(`[${new Date().toISOString()}] - taxonomy_interface - ERROR - ${msg}`),
    critical: (msg) => console.error(`[${new Date().toISOString()}] - taxonomy_interface - CRITICAL - ${msg}`)
};
class TaxonomyInterface {
    constructor(taxonomyFile, apiKey, model = 'gpt-4.1-nano', saveResults = false, outputFile) {
        this.sessionResults = [];
...
exports.TaxonomyInterface = TaxonomyInterface;
...
async function main() {
    ...
    try {
        ...
        const interface_ = new TaxonomyInterface(taxonomyFile, resolvedApiKey, model, saveResults, outputFile);
...
//# sourceMappingURL=interactiveInterface.js.map