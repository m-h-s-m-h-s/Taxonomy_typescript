/**
 * Example: Classifying products from the test dataset
 * 
 * This example shows how the Taxonomy Navigator works:
 * 1. Takes product name and description as input
 * 2. Generates an AI summary internally
 * 3. Returns the best matching Google taxonomy category
 * 
 * Usage: npx ts-node tests/classify-test-products.ts [number]
 * Example: npx ts-node tests/classify-test-products.ts 5
 */
...
function getRandomProducts(count: number): typeof testProducts {
...
async function classifyTestProducts() {
  ...
  if (isNaN(requestedCount) || requestedCount < 1) {
    ...
    console.log('Usage: npx ts-node tests/classify-test-products.ts [number]');
    console.log('Example: npx ts-node tests/classify-test-products.ts 5');
    return;
  }
